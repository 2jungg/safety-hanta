...
## 4. 덧붙이는 말
아래는 제가 정리하는 과정에서 떠오르는 것들을 생각나는 대로 적어둔 것입니다.
1. 현재 inference pod은 배치 준비랑 추론이랑 같은 파이썬 코드로 실행되고, subprocess로 분리되어 있습니다. 근데 테스트를 진행하는 과정에서 model을 메모리에 올리는 것이 매우매우 오래 걸립니다. 따라서 model을 메모리에 올리고, 배치로 된 입력을 받아 출력을 내는 pop을 따로 구축하는 것이 좋을 것 같습니다. 제가 간단히 적용해보려고 git의 test branch에 적용해 보았으나, 성공하진 못했습니다.
2. 참고로 저는 완성된 서비스를 만드려고 K8s를 사용했습니다만, cluster를 구성하고, node를 만들고, 이미지 로딩에 등등... 테스트하는 데 너무 많은 시간이 걸립니다. 따라서 현재 프로젝트에 있는 걸 다른 폴더에서 K8s를 제외하고 리팩토링 하시는 것도 괜찮습니다. dgx spark에서 서로 프로젝트 간의 간섭을 피하기 위해 docker를 사용하도록 하셨었는데요, 저는 개인적으로 docker 사용하면 너무 개발이나 디버깅 속도가 느려서 uv venv 기능을 사용하거나 conda 환경을 쓰더라도 충분하지 않나 생각합니다.
3. 해상도를 높이려면 configs/vision_config.yaml에서 픽셀 수를 높이시면 됩니다. 저 total pixels 값이 비디오 하나 당 추출하는 총 픽셀이니, 해상도를 계산하실 때 참고하시면 됩니다.
4. 추가로 코드나 코드 외적으로 궁금하신 점 있으시면 이중권(2jungg@kaist.ac.kr)에게 편하게 문의 주세요